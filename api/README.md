...
**Retrieval mode:** `HUB_HEAVY_RETRIEVAL=0` (default) uses BM25/keyword for DocChat and Food Search, text-context RAG for Icebreaker, and **vision-only Style Finder** (no ResNet/catalog) so small Render instances don’t OOM. Set `HUB_HEAVY_RETRIEVAL=1` on hosts with ~1GB+ RAM to enable MiniLM + vector stores and Style Finder catalog matching. `/health` reports `heavy_retrieval`.

**Icebreaker profile cascade:** ProxyCurl (`PROXYCURL_API_KEY` + LinkedIn URL) → pasted `profile_text` → bundled mock JSON.
