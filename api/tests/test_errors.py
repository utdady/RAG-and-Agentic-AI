...
def test_daily_usage_limit():
    ...
    assert err.title == "Demo usage limit reached"
    assert "99%" in err.message or "100%" in err.message
    assert "allowance" in err.message
    assert "refreshes around" in err.message
