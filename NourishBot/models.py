...
class NutrientBreakdown(BaseModel):
    ...
    vitamins: List[VitaminInfo] = Field(
        default_factory=list, description="List of vitamins and their %DV"
    )
    minerals: List[MineralInfo] = Field(
        default_factory=list, description="List of minerals and their amounts"
    )
...
class NutrientAnalysisOutput(BaseModel):
    ...
    estimated_calories: Optional[int] = Field(
        None, description="Estimated calories per portion"
    )
    nutrients: NutrientBreakdown = Field(
        default_factory=NutrientBreakdown, description="Detailed nutrient breakdown"
    )
